...
Build Debug `SwimTests` with `SWIM_ENABLE_VULKAN_RHI=ON` and `SWIM_BUILD_SHADER_COMPILER=ON` (both defaults). On a desktop with the required Vulkan feature baseline and validation layers, opt in to the twenty-eight native tests covering clear/transfer/presentation, triangle and texture readback, window/HDR lifecycle, timestamps, memory budgets, pipeline caches, upload/readback arenas, vertex/index/instance buffer drawing, push-constant updates, compute/storage-buffer readback, typed storage-image readback, entry-point descriptor readback, fixed descriptor-array readback, sampled integer/dimension readback, depth/comparison readback, RenderGraph rendering/compute/readback, graph-staged buffer/texture transfers, paged GeometryHeap uploads, TextureResidency mip-chain uploads, bindless texture/sampler indexing with timeline-safe element reuse, 100k-object GPU Scene dirty uploads probed by a compute shader, GPU visibility culling/LOD/binning drawn with `DrawIndexedIndirectCount`, and two-phase reverse-Z HZB occlusion culling:
