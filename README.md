...
`scripts\count-lines-of-code.bat` (or `count-lines-of-code.ps1`) reports Swim Engine's size. It prints files, code, comment and blank lines by area, by engine module, by file type, and the largest files. It skips build outputs, dependency caches and `Deprecated/`; pass `-IncludeDeprecated` to count that tree, or `-Top 25` to list more files.
...
The modern `Swim::Render` graph compiles declared passes into a deterministic dependency DAG, validates initialization, culls unused work, tracks mip/layer states, synthesizes barriers and pools compatible transient resources. `RenderGraphExecutor` runs graphics/compute/transfer work on one graphics queue, retains resources through timeline completion, and records per-pass labels/timestamps. Graph-declared upload/readback buffers are suballocated from executor-owned staging arenas, and `RenderGraphTransfers.h` adds whole/partial buffer and texture upload/readback passes. Dedicated queue ownership transfers and async scheduling remain later work.

Above the graph, `Renderer/Resources` provides generational GPU handles and a timeline-retiring `GpuResourceRegistry`, `Renderer/Geometry` provides the paged `GeometryHeap` (variable ranges in large device-local pages, stable per-mesh metadata and submesh rows, graph-recorded uploads, explicit residency and deferred frees), and `Renderer/Residency` streams cooked `.sasset` meshes and textures into it and into `TextureResidency` through async IO, job-side decoding and a per-frame upload budget (`AssetResidencyService`). `BindlessResourceTable` keeps one shared bindless texture/sampler table with fallback elements and timeline-retired index reuse, and `GpuSamplerCache` deduplicates samplers; resident textures become bindless explicitly through the residency service. See [GPU resource residency](docs/GpuResidency.md).

`Renderer/GpuScene` is the persistent render-object database: stable `RenderObjectHandle` rows in std430 instance/transform buffers (shared with Slang through `Shaders/Slang/GpuScene/GpuSceneRecords.slang`), dirty-only batched uploads, previous transforms for motion vectors and timeline-retired row reuse. It does not depend on EnTT; `Scene/RenderExtraction/RenderExtractor` is the EnTT producer, turning `MeshRenderer` components and the scene's transform dirty list into render-object updates. Both are exercised at 100k objects. See [GPU Scene and render extraction](docs/GpuScene.md).
...
Build Debug `SwimTests` with `SWIM_ENABLE_VULKAN_RHI=ON` and `SWIM_BUILD_SHADER_COMPILER=ON` (both defaults). On a desktop with the required Vulkan feature baseline and validation layers, opt in to the forty-one native tests covering clear/transfer/presentation, triangle and texture readback, window/HDR lifecycle, timestamps, memory budgets, pipeline caches, upload/readback arenas, vertex/index/instance buffer drawing, push-constant updates, compute/storage-buffer readback, typed storage-image readback, entry-point descriptor readback, fixed descriptor-array readback, sampled integer/dimension readback, depth/comparison readback, RenderGraph rendering/compute/readback, graph-staged buffer/texture transfers, paged GeometryHeap uploads, TextureResidency mip-chain uploads, bindless texture/sampler indexing with timeline-safe element reuse, 100k-object GPU Scene dirty uploads probed by a compute shader, GPU visibility culling/LOD/binning drawn with `DrawIndexedIndirectCount`, two-phase reverse-Z HZB occlusion culling, metallic-roughness materials shaded from the GPU material table with bindless textures, GPU-built image-based lighting (procedural sky, prefiltered specular cube, SH irradiance, BRDF LUT) checked against its CPU definitions, the PBR image-regression gallery compared per pixel with its CPU reference, brute-force PBR shading over a 2,000-light GPU light buffer, GPU clustered light assignment (cluster bounds, compact lists, overflow statistics, heatmap and clustered shading) checked against its CPU reference, Clustered Forward+ (opaque and sorted transparent draws, object ids, motion vectors under jitter, normal, indirect-light, specular-reflectance and specular-IBL targets and a heatmap view) compared pixel by pixel with an exact CPU ray cast, clustered-lighting GPU timings at up to 32k lights, cascaded/spot/point shadows (the read-back depth atlas against CPU shadow maps, the shadowed image against the CPU reference, and atlas eviction), the post stack (histogram, adaptive exposure, bloom levels and sRGB/HDR10/scRGB output compared stage by stage with the CPU reference), temporal anti-aliasing (every resolved texel of jittered, moving HDR frames against the CPU reference over the GPU's own history), screen-space GTAO, reflections and height fog (raw and blurred occlusion, the reflection march and the composite, stage by stage against the CPU reference), GPU particles (every particle, counter, draw list and indirect argument over 30 frames against the CPU simulation, and the drawn billboards pixel by pixel), and GPU skinning with morph targets (every skinned vertex and its previous-frame position of animated characters against the CPU reference, settling and pose bounds):
...
Without a GPU or window system (a container, CI), Mesa's lavapipe and SDL's offscreen video driver run every native case except the two window minimize/restore smokes. The synchronization and GPU-assisted profiles additionally need a Khronos validation layer 1.4.335 or newer on `VK_LAYER_PATH` (older distribution layers lack `VK_EXT_layer_settings`):

```bash
SDL_VIDEO_DRIVER=offscreen VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/lvp_icd.json \
SWIM_RUN_RHI_SMOKE=1 SWIM_RHI_VALIDATION=core ./build/linux-debug/SwimTests --filter=RHI.Vulkan.Smoke
```

The lifecycle test needs a window manager that supports minimize/restore. The timestamp test requires at least one timestamp-capable graphics/compute family, exercises eight reset/write/readback cycles on each supported queue role, and reports unsupported roles. Dedicated transfer-only families currently cannot provide the GPU query-reset lifecycle. Missing video/GPU support fails the opted-in cases; default tests include dispatch-capture and frame-lifecycle coverage without a GPU. Each smoke explicitly requires active validation and fails on unexpected warnings, errors, or dropped diagnostics, including resource/device/instance teardown. The only accepted warning is the exact reviewed GPU-AV startup advisory clamping `VkPhysicalDeviceDescriptorIndexingProperties::maxUpdateAfterBindDescriptorsInAllPools` to 4,194,304; its original warning severity/text and a separate advisory count remain in the report. Disabled instrumentation, feature adjustments, different limits, and all other warnings still fail. Debug regions and native object names are available to GPU tools when debug utils is supported. See [the architecture plan](docs/SwimEngineArchitectureImplementationPlan.md) for desktop evidence.
...
`core` is the default. `sync` adds synchronization and submit-time checks to core validation. `gpu` runs GPU-assisted validation as a separate pass with core checks off, as recommended by Khronos; `all` combines GPU-assisted and synchronization checks, also with core off. Always run `core` separately before `gpu`/`all`. Explicit checks require `VK_EXT_layer_settings` and a Khronos validation layer reporting API 1.4.335 or newer for synchronization; GPU-assisted validation requires 1.4.350 or newer so unsupported mesh/ray checks can be configured independently. GPU-assisted selection explicitly requires shader stores/atomics, shader integer widths, storage-buffer 8/16-bit access, scalar block layout, and the Vulkan memory model with device scope. Mesh/ray checks are disabled because those APIs are not enabled by Swim; normal core runs retain the existing device requirements. Missing support or an invalid profile fails the opted-in run.

On Windows, the wrapper builds the default suite, selects the newest available SDK layer from `VULKAN_SDK`, `C:\VulkanSDK`, or `.cache/vulkan-sdk/<version>/Bin`, and restores the caller's environment afterwards:

```powershell
scripts\run-debug-tests.bat -Validation core -VerboseCases -Report build\rhi-core.xml
scripts\run-debug-tests.bat -SkipBuild -Validation sync -VerboseCases -Report build\rhi-sync.xml
scripts\run-debug-tests.bat -SkipBuild -Validation gpu -VerboseCases -Report build\rhi-gpu.xml
scripts\run-debug-tests.bat -SkipBuild -Validation all -VerboseCases -Report build\rhi-all.xml
```

An existing `VK_LAYER_PATH` is respected. Pass `-VulkanSdkPath <SDK-root-or-x64-Bin>` to select a particular layer instead. The wrapper reports its version/path and rejects a discovered old SDK before launching advanced profiles. It performs no downloads, installation, or persistent environment changes. Install a current [LunarG Vulkan SDK](https://vulkan.lunarg.com/sdk/home) if needed; updating GPU drivers does not update the SDK's validation layer. The runtime also reports the detected layer version and settings support on setup failure.
...
The `DescriptorArrays.slang` smoke uses both elements of all six classes and verifies buffer/image readback after dependent compute passes. Its descriptor indices are compile-time constants. Fixed array allocation does not enable dynamic/non-uniform indexing for all resource classes; that requires the corresponding device features. Nested descriptor arrays remain separate work. See the architecture plan's fixed descriptor-array checkpoint for validation and scope.

### Bindless sampler/texture arrays

Unbounded `SamplerState Samplers[]` and `Texture2D<float4> Textures[]` declarations reflect as runtime-sized (`Count == 0`) bindings. The pipeline layout sizes them through `PipelineLayoutDesc::DescriptorSpaces`, an explicit space whose bindings set `PartiallyBound` and `UpdateAfterBind` (this requires `GraphicsCapabilities::BindlessDescriptors`). Such elements may be written after binding and while other elements are in flight, and a table binds to every pipeline that defines the space identically. Index them with `NonUniformResourceIndex`. `Render::BindlessResourceTable` manages the elements; the `Bindless.slang` smoke exercises the whole path. See [GPU resource residency](docs/GpuResidency.md#bindless-textures-and-samplers-item-45).
