...
Above the graph, `Renderer/Resources` provides generational GPU handles and a timeline-retiring `GpuResourceRegistry`, and `Renderer/Geometry` provides the paged `GeometryHeap` (variable ranges in large device-local pages, stable per-mesh metadata rows, graph-recorded uploads, explicit residency and deferred frees). See [GPU resource residency](docs/GpuResidency.md).
