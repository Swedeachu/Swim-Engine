...
namespace Swim::RhiVulkan
{
	...
	class VulkanAdapter final : public Rhi::Adapter
	{
	  public:
		...
		std::uintptr_t GetNativeHandle() const override { return ToNativeHandle(physicalDevice.physical_device); }

		const Rhi::AdapterInfo& GetInfo() const override { return info; }
		...
		std::unique_ptr<Rhi::Device> CreateDevice() override
		{
			...
			return std::make_unique<VulkanDevice>(
				std::move(deviceState), info, std::move(graphicsQueue), std::move(computeQueue), std::move(transferQueue));
		}
