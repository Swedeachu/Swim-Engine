...
namespace Swim::RhiVulkan
{
	...
	VkImageAspectFlags GetVulkanTextureViewAspect(Rhi::Format format, Rhi::TextureAspect aspect)
	{
		...
		case Rhi::TextureAspect::Color:
			return available & VK_IMAGE_ASPECT_COLOR_BIT;
		case Rhi::TextureAspect::Depth:
			return available & VK_IMAGE_ASPECT_DEPTH_BIT;
		case Rhi::TextureAspect::Stencil:
			return available & VK_IMAGE_ASPECT_STENCIL_BIT;
		default:
			return 0;
		}
	}
	...
	bool ValidateVulkanTextureView(const Rhi::TextureDesc& texture, const Rhi::TextureViewDesc& view, bool cubeArrays)
	{
		...
		if (GetVulkanTextureViewAspect(format, view.Aspect) == 0 || format != texture.PixelFormat ||
			...
			view.BaseArrayLayer >= texture.ArrayLayers || view.ArrayLayerCount > texture.ArrayLayers - view.BaseArrayLayer)
		{
			return false;
		}
