...
namespace Swim::Testing
{
		...
		: Coherent(coherent)
	{
		...
		functions.vkInvalidateMappedMemoryRanges = +[](VkDevice, std::uint32_t count, const VkMappedMemoryRange* ranges) -> VkResult
		{
			active->Invalidations.insert(active->Invalidations.end(), ranges, ranges + count);
			return active->InvalidateResult;
		};
	...
	VulkanMappedBufferCapture::~VulkanMappedBufferCapture()
	{
