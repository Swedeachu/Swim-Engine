...
namespace Swim::ShaderCompiler
{
	...
	std::string AppendRhiDescriptorBinding(
		const ShaderBindingReflection& parameter, Rhi::ShaderStageMask stages, Rhi::ShaderProgramInterface& interface)
	{
		if (parameter.HasUnsupportedBindingLayout || parameter.BindingKind != "descriptorTableSlot" || !parameter.HasIndex ||
			parameter.Count == 0)
		{
		...
		else if (typeKind == "resource" && parameter.ResourceAccess == "readWrite" && parameter.ResourceShape == "texture2D" &&
		...
		else if (typeKind == "resource" && (parameter.ResourceAccess.empty() || parameter.ResourceAccess == "read"))
		{
			if ((parameter.ResourceShape == "texture1D" || parameter.ResourceShape == "texture2D" ||
					parameter.ResourceShape == "textureCube" || (parameter.ResourceShape == "texture3D" && !parameter.ResourceArray)) &&
				!parameter.ResourceMultisample && parameter.ResourceComponentCount >= 1 && parameter.ResourceComponentCount <= 4 &&
				(parameter.ResourceScalarType == "float32" || parameter.ResourceScalarType == "uint32" ||
					parameter.ResourceScalarType == "int32"))
			{
				if (!parameter.ResourceFormat.empty() && parameter.ResourceFormat != "unknown")
				{
					return "Sampled textures require an unknown image format; the RHI binding describes numeric class, not an exact "
						   "storage format: " +
						parameter.Name;
				}
				...
				{
					sampledDimension =
						parameter.ResourceArray ? Rhi::TextureViewDimension::Texture1DArray : Rhi::TextureViewDimension::Texture1D;
				}
				else if (parameter.ResourceShape == "texture2D")
				{
					sampledDimension =
						parameter.ResourceArray ? Rhi::TextureViewDimension::Texture2DArray : Rhi::TextureViewDimension::Texture2D;
				}
				else if (parameter.ResourceShape == "textureCube")
				{
					sampledDimension =
						parameter.ResourceArray ? Rhi::TextureViewDimension::TextureCubeArray : Rhi::TextureViewDimension::TextureCube;
				}
				else
				{
				...
				if (parameter.ResourceScalarType == "uint32")
				{
					sampledClass = Rhi::SampledTextureClass::Uint;
				}
				else if (parameter.ResourceScalarType == "int32")
				{
					sampledClass = Rhi::SampledTextureClass::Sint;
				}
				else
				{
					sampledClass = Rhi::SampledTextureClass::Float;
				}
			}
		...
		if (std::any_of(schema->Bindings.begin(), schema->Bindings.end(),
				[&](const auto& binding)
				{
					return binding.Binding == parameter.Index;
				}))
		{
