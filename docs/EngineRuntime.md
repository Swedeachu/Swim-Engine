...
- **Look:** a tropical afternoon (`Sandbox::ApplyTropicalLook`): a saturated blue zenith, a bright cyan horizon and a turquoise "sea" below the horizon (no grey or brown band anywhere), a tight golden sun, the hue-preserving PBR Neutral tone mapper exposed bright (+1.3 EV over the auto-exposure average, stronger sky fill in the shadows) with a little warmth and saturation, and a thin blue sea-air haze. Screen-space reflections are on by default, and the sun's cascades are 4096 texels in an 8192 atlas (4 cm texels out to 70 m instead of 8 cm).
...
| Fixed | Ground collider shorter than the floor | The checkered plane is 130 m square but its box collider had 40 m half extents, so bodies fell through the outer 25 m of every edge (including under Sponza). Both now come from one `GroundSize`; `Game.Sandbox.TheGroundColliderSpansTheWholeCheckeredPlane` drops balls near all four edges. |
| Fixed | Particles erased near the horizon | Particles were drawn into the Forward+ color before the screen-space composite and the cloud feature: the fog re-fogged them with the depth *behind* them and the clouds were composited over them wherever sky showed through, so bands of a fountain vanished near the horizon. They are now drawn over the composited color, after the `BeforeTemporal` features. |
| Fixed | Sponza missing on a fresh checkout | The Draco + KTX2 Sponza GLB had been dropped from `Assets/Models/Sponza/` in a commit, so only the light swarm appeared. It is restored, and `verify-build-layout.py` now fails when it is missing and when an include's case does not match the file (`SandBox.h` broke Linux builds). |
