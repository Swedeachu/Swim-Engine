...
2. ~~Runtime wiring (with item 56)~~ *(done 2026-09-25, Phase 23)*: `UiRuntime` (`Engine/Runtime/UiRuntime.h`) owns the fonts, theme, atlas, `UiCanvasRouter` and `UiInputBridge`; the `UiCanvas` component (document, screen/world-panel/billboard mode, size, offset, units per pixel, pivot, billboard mode, constant screen size, fade, depth test, interactivity, pointer blocking, order, visibility) feeds it; the `FrameRenderer` draws the list with `UiRenderer`/`UiAtlasTextures`. The sandbox HUD and world UI use it; `FontPool` and the legacy text are archived. Still open: world canvases are composited after post (depth-tested, distance-faded) rather than sorted and fogged among the transparents, and `RenderSurface` canvases are not routed by the runtime yet. See [Engine runtime](EngineRuntime.md).
