...
4. **Hit test:** at each sample pixel the ray depth must lie within `[scene depth, scene depth + Thickness]`. Sky pixels and the start pixel never hit; leaving the screen is a miss.
5. **Refinement:** `RefineSteps` bisections of the last step move the hit to the first sample behind the depth buffer. A refined pixel on the sky (or the start pixel) falls back to the sample's pixel.
...
7. **Confidence:** `clamp((MaxRoughness − r) / RoughnessFade)` × screen-edge fade (hit pixel centre within `EdgeFade` of the border) × distance fade (the last `DistanceFade` of `MaxDistance`).
8. **Radiance:** the hit pixel's color, minus `(1 − ao) × indirect` there when AO is on, so a reflected corner is as occluded as the corner itself.
