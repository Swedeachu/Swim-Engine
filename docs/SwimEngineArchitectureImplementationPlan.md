...
## Current implementation snapshot — 2026-09-09
...
**Runtime integration status:** checkoffs below distinguish implemented contracts from migrated consumers and completed desktop validation. `Swim Engine.exe` still constructs the transitional `Engine::VulkanRenderer`; `--graphics=vulkan` does not select the new `Swim::RhiVulkan` path. Running the sandbox therefore does not close item **39**. The build scripts run default `SwimTests` cases; real-driver RHI smokes require `SWIM_RUN_RHI_SMOKE=1` and the documented validation profiles.
...
| Scene/physics | Active: catalog/runtime identities, scene command buffer, scene-owned transforms, per-view frustum, generic physics handles/bridge. PhysX is default; Jolt is selected explicitly when compiled. |
| Assets | Active cooked `.sasset` identity/load path and development auto-cook, with legacy engine-owned pools and `LegacyRenderBinding` still adapting assets for rendering. Async IO is available/injected; asynchronous GPU residency is not integrated. |
...
- **Latest RHI checkpoint — depth/comparison sampling:** depth-only sampling views, explicit comparison samplers, native depth-format checks and matching descriptor/barrier layouts are implemented. Slang JSON does not identify sampler comparison behavior, so callers configure `SamplerDesc` for the shader operation. One new readback smoke brings desktop coverage to nineteen opt-in cases. Desktop validation and sandbox migration remain open; see the Phase 9 checkpoint below.
...
| Modern graphics backend | `Systems/Renderer/RHI/Backends/Vulkan` | RHI clear/transfer, triangle/textured pipelines and explicit vertex/instance input are implemented. Item 39 still needs Windows/Linux GPU validation. It is not yet the game renderer. |
...
- [ ] Execute clear/triangle/texture/window smoke on Windows and Linux desktops with the full Vulkan baseline and validation layers. Record adapter/driver, window system, all four results, and clean validation diagnostics before checking item 39 or starting RenderGraph.
...
- [ ] Run all seven strict native GPU smokes on Windows/Linux desktops and retain adapter/driver, cache, memory, timestamp and post-teardown validation reports. The warm-cache smoke validates native creation and persistence; it does not claim warm-cache pixel parity or measured acceleration in this environment.
...
- [ ] Execute all eight native GPU smokes on Windows/Linux desktops and retain adapter/driver and validation evidence. Captured native calls exercise real VMA host logic but do not establish driver/GPU correctness.
...
- [ ] Run all ten native RHI smoke cases on Windows/Linux desktops, including the strict HDR mode on an HDR-enabled display. Retain actual selected format/color space, adapter/driver and post-teardown validation output. A prefer-HDR smoke passing on an SDR display proves the fallback path only.
...
- [ ] Run the ten-smoke desktop suite in core, synchronization and GPU-assisted modes on Windows and Linux; retain selected adapter/driver, layer version, requested checks and post-teardown diagnostics. Run the combined profile where useful and retain strict HDR evidence on a supported desktop. Captured loader calls and configuration tests are not native instrumentation evidence.
...
All profiles retain the existing core validation checks and require capture. Run separate profiles first to identify which checker reports an issue; GPU instrumentation and combined validation can increase execution time substantially. An opted-in smoke that exceeds its existing bounded deadline fails. It is not silently skipped. `SWIM_REQUIRE_HDR_SMOKE=1` remains independently available for the HDR lifecycle case.
...
- [ ] Run all **eleven** native RHI smokes on Windows and Linux desktops with the documented core, synchronization and GPU-assisted validation profiles. Retain adapter/driver, layer configuration and post-teardown diagnostics. Keep strict HDR coverage on an HDR-capable display. Only real passing evidence closes item 39.
...
- [ ] Run all **thirteen** native RHI smokes on Windows/Linux desktops with the documented validation profiles. Retain adapter/driver, validation requests and post-teardown diagnostics. Exercise dedicated-compute and graphics-aliased compute devices where available. CPU tests do not substitute for this gate.
...
- [ ] Run all **fourteen** native smokes on Windows/Linux desktops with the documented validation profiles; retain adapter/driver and post-teardown diagnostics. Exercise both dedicated-compute and graphics-aliased compute queues where available. Default deterministic tests are not native GPU evidence.
...
- [ ] Run all **fifteen** native smokes under the documented Windows/Linux desktop validation profiles and retain adapter/driver and teardown diagnostics. The new readback smoke supports a dedicated or graphics-aliased compute queue; actual GPU execution remains part of item **39**.
...
- [ ] Run all **sixteen** native smokes on Windows/Linux desktops with the documented validation profiles, retaining adapter/driver and post-teardown diagnostics. Test both dedicated-compute and graphics-aliased compute queues where available.
...
- [ ] Run all **seventeen** native smokes on Windows/Linux desktops with the documented validation profiles and post-teardown diagnostics. The sandbox executable and default capture tests are separate evidence.
...
- [ ] Run all **eighteen** native smokes on Windows/Linux desktops with the documented validation profiles and post-teardown diagnostics. Cube-array coverage is reported explicitly when supported; the base shapes remain required otherwise.
...
- [ ] Run the new depth smoke and all **nineteen** native smokes on supported Windows/Linux desktops with the documented validation profiles. Passing default capture tests or running the sandbox does not close this gate.
...
**Following work:** close **39** with real desktop evidence before **40**. Nested resource layouts/parameter blocks are the next independent RHI contract; runtime-sized/bindless policies and automatic vertex-interface validation remain open. No legacy technology is retired by this checkpoint. Keep top-level `Deprecated/`, preserve build scripts, and deliver a full clean repository ZIP with fresh file timestamps and no generated caches or migration scripts.
...
- [ ] resize/minimize/restore loop is stable on Windows and Linux. *(Implementation, native-dispatch tests and opt-in desktop smoke are present; desktop validation is pending.)*
...
39. [ ] Validation-clean RHI clear/triangle/texture on Windows and Linux. *(Clear/transfer and Slang procedural/indexed triangle pipelines plus opt-in pixel/presentation smoke are implemented. Reflected fixed-count descriptors, samplers and sampled 2D texture drawing are implemented. Resize/minimize/restore handling, cancellation, and opt-in lifecycle smoke are implemented. Required validation capture, native names/labels, adapter reports and strict post-teardown smoke checks are implemented. GPU timestamp pools, queue precision, reset/write/readback and strict reuse smoke are implemented. Device-loss reports, optional bounded fault capture and lost-device retirement are implemented. Memory-budget snapshots, allocator counters and strict allocation/release smoke are implemented. Shared native pipeline caches, bounded persistence data and cold/warm recreation smoke are implemented. Persistent upload/readback arenas and HDR surface negotiation with strict/fallback lifecycle smoke are implemented. Explicit GPU-assisted/synchronization-validation settings and selectable smoke profiles are implemented. Explicit vertex layouts, validated vertex/instance buffer bindings and direct/indexed pixel-parity smoke are implemented. Reflected graphics push-constant ranges, validated partial updates, compatible-layout persistence and pixel smoke are implemented. Reflected compute pipelines, writable storage buffers, dispatch/barriers and a strict two-pass readback smoke are implemented. Typed 2D storage-image reflection/descriptors, native format/usage checks, compute-family image transfers and a float/uint/int subresource readback smoke are implemented. Direct entry-point descriptor reflection, stage-specific visibility, collision rejection and a two-space compute readback smoke are implemented. Fixed descriptor-array reflection, native element/bounds/initialization regression coverage and a six-resource-class array readback smoke are implemented. Float/Uint/Sint sampled 2D reflection and native type validation, plus a typed texel-load readback smoke, are implemented. Real Windows/Linux GPU execution remains open. See the September 2026 Phase 9 checkpoints.)*
