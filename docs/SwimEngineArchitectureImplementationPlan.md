...
| Text/UI | `Systems/Text` (font faces, fallback chains, bidi/script/line-break paragraph layout, carets, MSDF atlas with parallel prewarm), `Systems/UI` (retained flex/anchor layout with measure/paint caches, images, editing and IME, themed controls — buttons, checkboxes, toggles, sliders, scroll bars — with visual states, transitions, Tab/spatial/gamepad navigation, canvas placement math and multi-canvas input routing), `Systems/UiInput` (Input → UI bridge, gamepad) and `Renderer/UiRendering` (one-draw RenderGraph UI pass for screen and world canvases with depth testing, atlas residency, render surfaces with mip chains, SDR/HDR composition; item 79) are tested and available to consumers. Nothing in the sandbox constructs them yet; legacy text/UI remains active until the modern renderer presents frames (item 56). |
