...
| GPU residency | `Systems/Renderer/Resources`, `Systems/Renderer/Geometry`, `Systems/Renderer/Residency` | New backend-neutral layers (items 42–44); no legacy consumer yet. `MeshPool`/`TexturePool`/`VulkanIndexDraw` paths stay active until the modern renderer draws from `GeometryHeap`/`TextureResidency` and the engine constructs `AssetResidencyService`; only then can those pools be archived. |
...
- [ ] Execute the new native smoke on Windows and Linux desktops under all validation profiles.
...
GPU residency layers have their own groups: `RenderResources` (`Render.GpuResourceRegistry`), `RenderGeometry` (`Render.GeometryRangeAllocator`, `Render.GeometryHeap`) and `RenderResidency` (`Render.MeshGeometryPayload`, `Render.TextureResidency`, `Render.AssetResidency`, compiled with the IO/Platform foundation). Cases that cook real `.sasset` objects live in the `AssetCompiler` group.
