...
**Following work:** critical-path **39** still requires real Windows/Linux desktop evidence before **40** (RenderGraph). GPU-assisted/synchronization-validation configuration is another remaining Phase 9 implementation task. Item **41** still separates the implemented arenas/direct transfers from graph-scheduled integration. No legacy sources are retired here. Preserve the consolidated build and top-level `Deprecated/`; ship a complete clean repository with no generated build/dependency caches or cleanup/migration scripts.
