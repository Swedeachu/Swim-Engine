...
- [x] Resume item 39 with the Slang-backed triangle pipeline checkpoint below. Sampled textures and real Windows/Linux GPU validation remain open; the current game still uses the transitional renderer.
...
- [ ] Add descriptor-bearing reflected layouts, descriptor tables/writes, samplers, and sampled-texture drawing.
...
- [x] Create pipeline layouts from program-owned reflection. The triangle path requires an empty descriptor/push-constant interface; nonempty interfaces fail explicitly until resource bindings are implemented. The program must outlive its layout because `GetProgram()` returns a reference.
...
- [ ] Implement reflected descriptor layouts/tables/writes and samplers, then upload and sample a texture through this pipeline path. Add explicit vertex-input layout contracts/bindings as needed; the current graphics path uses shader-generated vertices, and `BindVertexBuffer` remains unsupported.
...
39. [ ] Validation-clean RHI clear/triangle/texture on Windows and Linux. *(Clear/transfer and Slang procedural/indexed triangle pipelines plus opt-in pixel/presentation smoke are implemented. Descriptor/sampler/textured drawing and real Windows/Linux validation remain open. See the 2026-09-05 Phase 9 checkpoints.)*
