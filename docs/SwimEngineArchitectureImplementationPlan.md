...
- **Phase 1 — Platform foundation:** the SDL3-backed `Swim::Platform`/`Swim::Input` foundation, normalized window/input types, filesystem/mapped-file/dynamic-library APIs, native-window escape hatch, headless path, and generic-PCH cleanup are implemented. The old Win32 external-editor/`WM_COPYDATA` bridge remains in the Platform source tree only as dormant reference code and is not initialized by `SwimEngine`; future editor work is in-process engine UI. The remaining Phase 1 gates are runtime smoke coverage for the same `HelloWindow` API on both Windows and Linux and explicit Windows public-header validation. The legacy `InputManager` still adapts `Swim::Input` for old gameplay code, so the final gameplay-facing input migration is not complete.
...
- [ ] Add shader modules, reflected pipeline layouts/descriptors, samplers, graphics pipelines, and draw recording for the Slang triangle and sampled-texture smoke.
...
To run the current clear/transfer smoke on a desktop with the full required Vulkan feature baseline, build the Debug `SwimTests` target and enable the opt-in case. Run with the Vulkan validation layers installed; inspect the validation output as well as the pixel checks. This is the clear/transfer gate only, not yet the triangle/texture gate.
...
Backends/Vulkan/
  ...
  Resources/    VulkanBuffer, VulkanTexture, VulkanTextureView
...
39. [ ] Validation-clean RHI clear/triangle/texture on Windows and Linux. *(Clear/transfer commands and opt-in pixel/presentation smoke are implemented; triangle/textured pipelines and real Windows/Linux validation remain open. See the 2026-09-05 Phase 9 clear/transfer checkpoint.)*
