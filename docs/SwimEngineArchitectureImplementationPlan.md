...
- **Latest RHI checkpoint — sampled image dimensions:** reflected 1D/array/2D/volume/cube color views now retain shader dimensionality alongside Float/Uint/Sint classes. Cube arrays are optional and capability-gated. Native view creation and descriptor writes share shape/range validation. The new compiled readback smoke brings desktop coverage to eighteen opt-in cases; desktop validation and sandbox migration remain open. See the detailed Phase 9 checkpoint below.
...
**Following work:** close item **39** with desktop evidence before RenderGraph. Depth/comparison sampling is the next independent sampled-image contract; nested layouts and runtime-sized/bindless policies remain open. Preserve consolidated engine targets and top-level `Deprecated/`. No legacy source is retired by this checkpoint. Deliver the complete clean repository with refreshed ZIP timestamps and no generated dependency/build output or migration scripts.
