...
>
> **Rendering direction:** Vulkan is the modern graphics backend and, since Phase 22/23, the only one. D3D12 and Metal are future RHI backends. The OpenGL renderer was retired in Phase 22 (archived under `Deprecated/`).
>
> **Physics direction:** gameplay talks only to Swim physics contracts. PhysX and Jolt are selectable backend implementations.
>
> **Shader direction:** Slang is the canonical source language/compiler stack for all first-party shaders. Vulkan consumes Slang-generated SPIR-V (the runtime set is deployed as `Shaders/Runtime`). Future D3D12 consumes DXIL. Metal support is added when the Slang/Metal path is sufficiently mature.
...
**Runtime integration status (Phases 22 and 23, 2026-09-25):** `Swim Engine` now runs the modern stack end to end. The RHI Vulkan device and swapchain (or headless target) feed a `FrameRenderer` that owns one RenderGraph per frame: residency, GPU scene, GPU visibility, Forward+, shadows, environment, post, TAA, screen-space effects, particles, skinning and UI. The sandbox demo scene exercises all of them from a UI control panel. OpenGL, the editor, the transitional `Engine::VulkanRenderer` and the legacy pools are archived under `Deprecated/`. See [Engine runtime](EngineRuntime.md). The assembled runtime is validated on Linux with SwiftShader (headless captures and the headless engine tests; see the [validation record](validation/Phase22-23-2026-09-25.md)). Desktop validation under the four profiles, Windows builds of the new runtime and the 1080p budgets are still open.

| Area | Runtime status |
| --- | --- |
| Platform/Input/configuration | Active: SDL windows and events, headless offscreen video, one accepted-frame input snapshot, explicit engine-owned services, and runtime configuration including state, clock, automation (`--frames`, `--fixed-delta`, `--capture`, `--exec`) and `--no-render`. |
| Engine state and time | Active: `EngineStateMachine` (Playing / Paused / Stopped with ordered listeners) and `SimulationClock` (fixed steps, time scale, pause, single steps, per-domain participation, spiral guard). |
| Scene/behaviours/tags | Active: scene catalog and ids, command buffer, scene-owned transforms, behaviours with state masks, real-time behaviours and state hooks, hashed multi-tags with a scene index, names, Stop reset through `SceneSystem`. The fly camera is a runtime behaviour. |
| Physics | Active: generic physics with PhysX or Jolt (`--physics`), fixed-step collision callbacks to behaviours, and raycasts resolved to entities. |
| Jobs/memory/IO/assets | Active: enkiTS jobs, mimalloc, frame arena, async IO, the cooked `.sasset` path, and `AssetResidencyService` streaming procedural and cooked meshes and textures into `GeometryHeap`/`TextureResidency`. |
...
2. ✅ (runtime and demo in place; desktop validation open) **Assemble the modern engine and the sandbox demo** ([Phase 23](#phase-23--engine-assembly-and-the-sandbox-demo)): the runtime constructs the new systems (RHI Vulkan, RenderGraph, GPU Scene, visibility, Forward+, shadows, post, TAA, particles, skinning, UI), the transitional `Engine::VulkanRenderer` and legacy pools go away, and one real sandbox scene demonstrates everything with UI controls: a physics and rendering playground, lighting and shadows, play/pause/step, the same movable camera. It shows where the engine stands and what needs fixing.
...
- **Previous checkpoint — item 79, controls, themes and world-space canvases (2026-09-25):** the two Phase 20 checklists added after the previous checkpoint (controls with theming; world-space and render-surface canvases) are implemented, except the scene/ECS canvas component and fog on world UI, which move to the runtime wiring with item 56. Item 79 stays open until both native UI smokes pass on the desktop and the runtime draws its UI through them.
  ...
  - **Canvases** (`UiCanvas`, `UiCanvasRouter`, both in `Systems/UI`, renderer-independent): screen, render-surface, world-panel and billboard (spherical, cylindrical, screen-aligned, constant screen size, distance fade) placement; screen rays, ray–canvas hits and projections; a router that picks the nearest canvas under the pointer (screen first, then world panels and application surface hits), keeps pointer capture on the canvas plane, and gives keyboard/text/IME to one canvas at a time with projected IME rectangles. `UiInputBridge::Apply(input, router, camera, …)` drives it.
  - **Rendering** (`Renderer/UiRendering`, `SwimUiQuad`): the draw constants carry a canvas-to-clip matrix (96-byte push constants); world canvases take coverage ramps and MSDF ranges from derivatives, test scene depth (reverse-Z, a depth pipeline variant) and fade by canvas opacity; frames can target one mip. `UiRenderSurfaces` keeps bindless surface textures and redraws every mip (re-rasterized per level) only when the document's paint revision changes. The CPU reference gained `CanvasAt`/`RasterizeProjected`. GPU-AV budget: 13 accesses.
  - **Text fix:** point-only contours and zero-length lines are dropped before msdfgen; an empty contour drew a streak through open glyph tops (visible above the fixture's `u`).
  - **Validation:** the Linux configuration passes **765 cases / 658,001 checks** (was 740 / 657,513); 25 cases are new (82 in the focused Text/UI/rendering filter), and the focused modules pass under ASan/UBSan/LSan (81 cases). On a source-built SwiftShader, the screen smoke and the new `RHI.Vulkan.Smoke.UiWorldCanvasesMatchTheCpuReference` (orthographic and perspective world panels, a billboard, depth occlusion, canvas fade, themed controls, a render surface's mips 0 and 2 and a panel sampling it) compared 518,708 pixels with **0 outliers**. See [the controls and canvases record](validation/Item79-Controls-2026-09-25.md).
  - **Next:** run both UI smokes on the RTX 4070 under all four validation profiles and record the 1080p budget; then runtime wiring with item 56 (scene canvas component, world UI among transparents with fog, sandbox migration). Contracts and commands are in [Text and retained UI](TextAndUi.md).
- **Previous checkpoint — item 79, paragraphs, editing and UI rendering (2026-09-25):** the rest of Phase 20's runtime at that point.
  - **Text** (`Systems/Text`, two new pinned private dependencies: SheenBidi `v3.0.0`, libunibreak `8.0`):
    - `FontCollection`: ordered fallback chains, per grapheme cluster (neutral characters keep the previous face; ignorables never force a face).
    - `TextSegmentation`: UAX #29 graphemes/words, UAX #14 break opportunities, UAX #24 script runs, UAX #9 paragraphs and levels. `Utf8` decodes and sanitizes.
    - `TextLayout`: paragraphs → items (level, script, face) → measuring shape → greedy line breaking (hanging spaces, emergency grapheme breaks, U+2028) → per-line L1–L2 reordering and shaping → per-line metrics → Start/End/Left/Right/Center alignment. Caret, hit-test, selection rectangles and grapheme/word/line movement queries.
    - `GlyphAtlas::Prewarm` builds distance fields through a caller's parallel-for and packs deterministically (pixel-identical to sequential `Get`); `FontFace` now serializes only outline extraction.
  - **UI** (`Systems/UI`, split into tree/layout/paint/input/editing units):
    - Flex grow/shrink, justification and cross alignment (stretch); anchors and pivots; aspect ratios; wrapping paragraph text with alignment and line spacing; images (stretch/contain, nine-slice); rounded and bordered backgrounds.
    - Paint-only style changes skip layout; measurement is cached per subtree; paint is cached per node.
    - Editable text: grapheme/word/line navigation, selection, clipboard callbacks, `MaxBytes`, submit, IME preedit with underline, caret reveal in clipped fields, `GetTextInputRect`. Wheel scrolling. A widget layer (`UiWidgets`) with button visual states.
  - **Input:** `Systems/UiInput/UiInputBridge` applies an `Input::InputSystem` frame to a document (DPI, press/release, wheel, key repeats, ordered text, composition, focus loss). `InputSystem` gained `GetKeyPresses`, `GetTextEditEvents` (keys and text in event order) and `HasTextCompositionUpdate`; `WindowSystem` gained `SetTextInputArea`.
  - **Rendering** (`Renderer/UiRendering`, `Shaders/Slang/Ui`, `SwimUiQuad`): `UiRenderer` records one pass with one instanced draw of 112-byte `GpuUiQuad`s in paint order — per-instance clipping in the vertex stage, bindless glyph pages and images, rounded/bordered SDF solids, MSDF glyphs with CPU-computed pixel ranges, premultiplied blending — composited onto sRGB, linear (`*Srgb`/float), HDR10 (PQ) or scRGB targets with a paper-white UI level. `UiAtlasTextures` uploads new pages whole and later only the changed row band, with commit/abort and timeline-retired pages. `UiRenderReference` (`Ui::`) is the CPU definition; `ShaderCompiler.UiLayout` pins the record layout; the GPU-AV budget counts 12 accesses.
  - **Validation:** the Linux configuration passes **740 cases / 657,513 checks** (was 704 / 656,771 in this container; the documented Windows default suite was 714). 36 cases are new (57 in the focused Text/UI/rendering filter); the focused set also passes under AddressSanitizer/UndefinedBehaviorSanitizer/LeakSanitizer. `verify-build-layout.py` passes with the new boundary rules, each checked to fire on a planted violation. The native smoke `UiRendererMatchesTheCpuReference` ran on a source-built **SwiftShader** (no lavapipe in this container) with a draw-parameters-free shader variant: 159,840 pixels over five frames (sRGB, HDR10, scRGB, linear→`*Srgb`, grown atlas) with **0 outliers** (worst 6.3·10⁻³). That run found and fixed two bugs: the vertex stage ignored the RHI's +Y-up clip space (the image was flipped), and the float-target tolerance did not scale with scRGB white. See [the item 79 record](validation/Item79-2026-09-25.md).
  - **Next:** run the native smoke on the RTX 4070 under all four profiles and record the 1080p UI budget; then the themeable control set (scroll bars, checkboxes, toggles, sliders, focus navigation) and world-space canvases (render surfaces, world panels, billboards, ray-cast input and shared focus) — see Phase 20; then runtime wiring with item 56. Contracts and commands are in [Text and retained UI](TextAndUi.md).
- **Previous checkpoint — item 79, text and retained UI foundation (2026-09-24):** the first runtime portion of Phase 20.
  ...
  - **Cleanup (2026-09-25):** style validation, content boxes and glyph quads in `UiDocument` are split into named helpers; `FontFace` rasterization is split into outline→shape and bitmap-encoding helpers; `UiEvent::Kind` is default-initialized. `GlyphAtlas::GetChangedRows` reports the rows written since a revision (new `Text.Atlas` case) for partial page uploads. 21 Text/UI cases pass.
  - **Followed by:** the paragraph, editing and UI rendering checkpoint above.
...
**Target:** FreeType + HarfBuzz + MSDF atlas generation/caching, plus a retained runtime UI system with its own layout and hit-testing model. It needs a complete, themeable control set (buttons, text fields, scroll views and scroll bars, checkboxes, toggles, sliders) and focus navigation. Any UI document must also be displayable as a screen overlay, a render-surface texture, a world-space panel or a billboard, with pointer, keyboard and focus input in every mode.
...
| SheenBidi | `v3.0.0` unity TU (`Source/SheenBidi.c`) | `SwimSheenBidi` -> `Swim::TextDependencies` | yes, once the text module ships | Unicode Bidirectional Algorithm (paragraphs, levels, per-line L1–L2 runs) and UAX #24 script runs, over UTF-8 byte offsets. Apache-2.0. |
| libunibreak | `libunibreak_8_0` library sources (Makefile.am set) | `SwimUnibreak` -> `Swim::TextDependencies` | yes, once the text module ships | UAX #14 line breaking and UAX #29 grapheme/word boundaries (Unicode 17). zlib licence. |
...
- [x] Construct `RenderExtractor` + `GpuScene` in the engine runtime and draw from them (Phase 13 visibility, then item 56 retires the CPU visible list). *(2026-09-25, Phase 23: `SceneRenderBridge`.)*
...
Font faces, run shaping, the MSDF atlas and the retained `UiDocument` foundation.

### Paragraph, editing and UI rendering checkpoint — 2026-09-25

See [Text and retained UI](TextAndUi.md) for contracts, commands and remaining work, and [the item 79 record](validation/Item79-2026-09-25.md). Item 79 is **implemented but open**: the native smokes have not run on the desktop yet, and no runtime path constructs the UI renderer until the modern renderer presents frames (item 56).

### Controls, themes and canvases checkpoint — 2026-09-25

The control set with theming and the world-space/render-surface canvases (the two sections below) are implemented; see [the controls and canvases record](validation/Item79-Controls-2026-09-25.md). Left for the runtime wiring with item 56: the scene/ECS canvas component, and world UI sorted and fogged with the scene's transparents.
...
- [x] Widget layer: images (`SetImage`, `CreateImage`, stretch/contain, nine-slice), text (`CreateLabel`), buttons (`CreateButton`), panels, scroll views and scroll areas, input fields (`CreateTextField`, `SetEditable`), checkboxes, toggles and sliders — themed, with visual states. *(Nodes plus document-implemented controls, not a separate widget class hierarchy.)*
- [x] Platform/Input adapter (`Systems/UiInput/UiInputBridge`; `InputSystem` text-edit events and composition updates; `WindowSystem::SetTextInputArea`).
- [ ] Sandbox migration. *(Needs the modern renderer to present frames, item 56; legacy text/UI stays active.)*

### Controls (checkpoint 2026-09-25)

Every control is built from ordinary document nodes and events and is driven only through `UiDocument` input (pointer, wheel, keys, focus, activation). None reads the platform or `InputSystem` directly, so the same control works on a screen canvas and on a world-space canvas. Contracts: [Text and retained UI — Controls](TextAndUi.md#controls).

- [x] **Scroll bars:** vertical and horizontal, attached to any clipped node (`ScrollTarget`). Thumb size follows the viewport/content ratio (with `MinThumbLength`). Thumb drag with pointer capture; track press pages (or jumps); optional step buttons that repeat while held; wheel over the bar; arrows, Page Up/Down, Home/End when focusable. Visibility: `Always`, `Auto` (only on overflow) and `Overlay` (fades after inactivity). The bar re-syncs in the same Layout when content, viewport or offset changes from code. `CreateScrollBar`, `CreateScrollArea`.
- [x] **Checkboxes:** unchecked, checked and mixed (from code; a click checks it). Click, Space or Enter; the label is part of the hit target; `ValueChanged`/`ValueCommitted`.
- [x] **Toggles (switches):** on/off with an eased knob (`Update`). Click, knob drag (past half on release) or Space/Enter; the same event contract.
- [x] **Sliders:** horizontal and vertical (Min at the bottom); float or stepped values. Thumb drag with capture and grab offset, track press `Jump` or `Page`, wheel while focused, arrow/Page/Home/End keys (cross-axis arrows navigate). Tick marks (`Ticks`, `SetPartRole`) and a value label written by the document (`LabelDecimals`). `ValueChanged` while dragging, `ValueCommitted` on release.
- [x] **Shared control contract:** `ValueChanged`/`ValueCommitted` with `UiEvent::Value`; `SetValue`/`SetChecked` from code without events; disabled (no hover, press or focus; the theme's disabled look) and read-only states; focus rings through state rules.
- [x] **Focus navigation:** Tab/Shift+Tab with `TabIndex` (positive first ascending, then document order; negative skipped), spatial `Navigate` for arrows and the gamepad D-pad/stick (hold-repeat), activation with Enter/Space or gamepad A (toggles checkboxes and toggles), Escape/gamepad B blurs, shoulders tab. The bridge maps the gamepad like the keyboard.
- [x] **Customization and theming:**
  - every control exposes its parts (track, fill, thumb/knob, ticks, check and mixed marks, label, step buttons) as normal nodes (`GetControl(node).Parts`);
  - per-state styles are layered `UiStateRule`s (hover, pressed, focused, disabled, checked, mixed, read-only, dragging; `When`/`Unless` masks), replacing `UiButtonColors`/`UiButtonStates`;
  - `UiDocument::SetTheme(UiTheme)`: palette, metrics (radii, borders, focus width, spacing, paddings, control dimensions, text sizes), font chain and a `Customize` hook; `SetThemeClass(node, class, Paint|Layout|Text)`; per-node rules override the class;
  - image/nine-slice skins for any node through rule `Image`s (paint-only);
...
A `UiDocument` stays a 2D canvas in framebuffer pixels. Where it is shown is decided by a placement (`UiCanvas.h`), the renderer and one input router, not by the document, so every control and widget works unchanged in each mode. Contracts: [Text and retained UI — Canvases](TextAndUi.md#canvases-uicanvash).

- [x] **Canvas placement modes:**
  - **Screen:** the overlay on the swapchain or any color target (`ScreenClipFromCanvas`, `UiRenderFrame`).
  - **Render surface:** `UiRenderSurfaces` renders a document into its own color texture (size, format, DPI, clear color per surface), registered in the bindless table, so any material, mesh or post pass can sample it.
  - **World panel:** `CanvasToWorld(WorldPanel, placement)` — transform, pivot, world units per canvas pixel. Drawn directly (`UiRenderFrame::ClipFromCanvas`, depth tested) or through a render surface on a quad (`UiRenderSurfaces::PanelPaint`; fixed cost, every mip re-rasterized from the document).
  - **Billboard:** spherical, cylindrical (around an up axis) or screen-aligned; constant screen size; distance fade (`CanvasFade` → `UiRenderFrame::Opacity`); depth-tested or always on top (no depth).
- [x] **Rendering requirements:** the quad shader maps canvas pixels through a per-canvas `ClipFromCanvas` in the draw constants while keeping per-instance clipping in canvas space; world canvases take MSDF pixel ranges and coverage ramps from derivatives; depth testing against reverse-Z scene depth (a depth pipeline variant, no writes); HDR composition per canvas (`Linear` with `LinearScale`, paper white for HDR10/scRGB); one draw per canvas; render surfaces redraw only when the document's paint revision changes. *(Sorting among the scene's transparents and fog belong to the runtime wiring with item 56: `CanvasDistance` gives the sort key; world UI applies no fog yet.)*
- [x] **Input and focus in world space** (`UiCanvasRouter`, `UiInputBridge::Apply(input, router, camera, surfaceHits)`):
  - a mouse ray (`ScreenRay`), crosshair or VR/controller ray is intersected with panel and billboard quads; render surfaces on arbitrary meshes take the application's hits (mesh ray cast → UV → canvas pixels, `UiSurfaceHit`);
  - the nearest hit canvas (screen canvases first) receives pointer moves, presses and the wheel in its own pixels; the previous one gets `PointerLeave`;
  - pointer capture follows the pressed canvas on its unbounded plane, even when the ray leaves the panel;
  - one focus owner across canvases receives keys, text and IME; focusing another canvas blurs the previous one; the IME rectangle is the projected caret;
  - canvases can be non-interactive, and `BlocksPointer` decides whether empty canvas areas stop the pointer.
- [ ] **API/ownership:** the scene/ECS canvas component (document handle, placement mode, transform or target surface, size, DPI, interactivity) moves to the runtime wiring with item 56. Placement math, ray casting, projection and routing live in `Systems/UI` (`UiCanvas`, `UiCanvasRouter`) — renderer-, scene- and platform-independent, as the verifier enforces — and world drawing in `Renderer/UiRendering`.
- [x] **Tests:** `UI.Canvas` (4: screen mapping, rotated/scaled panels with back-face and edge hits, billboards with constant size and fade, ray round trips for perspective and orthographic cameras), `UI.CanvasRouter` (3: stacking and blocking, nearest hit and capture off the panel, focus handoff with projected IME rectangles), `Render.Ui.*` world/depth/mip/surface cases, and the native smoke `UiWorldCanvasesMatchTheCpuReference` (world panels through orthographic and perspective cameras, a billboard, depth occlusion, canvas fade, themed controls, render-surface mips 0 and 2, a panel sampling the surface) compared with `Ui::RasterizeProjected`/`Ui::Rasterize`.

### Layout

- [x] measure -> layout -> paint;
- [x] parent/child hierarchy;
- [x] anchors; *(min/max anchors with pivots for absolute children; stretch between different anchors)*
- [x] margin/padding;
- [x] min/max/preferred size;
- [x] row/column stacks and overlay layout; flex grow/shrink, justification and cross alignment/stretch; *(no wrapping flex lines, no baseline alignment)*
...
- [x] scrolling/clipping; *(wheel scrolling of the innermost scrollable clip)*
- [x] document-level dirty layout invalidation and reuse of unchanged shaped text;
- [x] subtree-only layout invalidation and cached paint-list updates. *(Measure results are cached per subtree and paint per node; paint-only style changes skip layout. Arrangement still visits every node.)*
...
- [x] font fallback; *(ordered `FontCollection` chains per grapheme cluster; never an OS font)*
- [x] homogeneous RTL/script runs; *(Arabic contextual shaping covered)*
- [x] paragraph bidi/script segmentation; *(UAX #9 via SheenBidi, including L1–L2 per line; UAX #24 script runs)*
- [x] Unicode line breaking/wrapping; *(UAX #14 via libunibreak; hanging spaces, emergency grapheme breaks, alignment)*
- [x] selection/caret; *(grapheme/word/line movement is logical; ligatures split carets)*
- [x] IME visualization; *(preedit at the caret with underline, candidate-window rectangle)*
- [x] MSDF atlas pages, bounded packing, stable UVs and page revisions;
- [x] GPU uploads/residency and timeline-safe atlas retirement (`UiAtlasTextures`); parallel atlas prewarm (`GlyphAtlas::Prewarm`, `UiDocument::PrewarmGlyphs`);
- [ ] background (cross-frame) atlas population and atlas eviction.

### Rendering

- [x] Backend-neutral ordered solid/glyph/image quad output with clip rectangles, UVs and MSDF distance range;
- [x] batched instanced quad RenderGraph pass; *(one draw per document)*
- [x] bindless images/glyph atlases;
- [x] rounded/SDF primitives;
- [x] borders/nine-slice;
- [x] GPU clipping/scissor indexing; *(per-instance clipping of axis-aligned quads in the vertex stage; clips are rectangles)*
- [x] premultiplied linear alpha paint policy;
- [x] HDR-aware composition (sRGB, linear/`*Srgb`, HDR10 PQ, scRGB with paper white);
- [x] world-space canvases: canvas-to-clip matrices, derivative-based coverage and MSDF ranges, scene depth testing, canvas fade, mip targets, render surfaces (`UiRenderSurfaces`);
- [ ] native image-regression smokes passing on the desktop. *(`RHI.Vulkan.Smoke.UiRendererMatchesTheCpuReference` and `UiWorldCanvasesMatchTheCpuReference` pass on SwiftShader in the container; not yet executed on a GPU under the validation profiles.)*

World-space text and world-space UI both reuse the shaping/atlas services and the same retained document; see *World-space and render-surface canvases* above. The current sandbox still uses legacy text/UI; this checkpoint does not rewire it.
...
**Done 2026-09-25** (see [Engine runtime](EngineRuntime.md)). Decided 2026-09-25 after item 79's widgets ( supersedes the earlier plan to keep OpenGL as an isolated legacy backend and to preserve editor systems as tooling). The modern RHI path is the engine; carrying a second renderer and editor-era systems into the assembly phase would double every migration.

### Remove

- [x] **OpenGL renderer** (`Systems/Renderer/OpenGL`, GL loaders and dependencies, `--graphics=opengl`, `GraphicsBackend::OpenGLLegacy`, GL branches in shared types, GL shader outputs in the Slang build). The verifier rejects GL includes and symbols afterwards.
- [x] **Editor systems:** gizmos (`GizmoSystem`, `ChangeGizmoTypeButtonBehavior`), the editor camera as an editor concept, editor-only commands, editor scene bookkeeping, and `Editing`-state branches in systems. The archived IPC (`Deprecated/`) stays archived.
- [x] Generic `Vertex`/`Texture`/`Transform`/`Camera` types lose remaining backend behavior (item 83).
- [x] Tests, examples and docs that exercise removed code are deleted or rewritten; nothing is kept "for later" behind flags.

### Keep and improve (runtime features the editor used to host)

- [x] **Engine state:** `Playing`, `Paused`, `Stopped` as an explicit state machine with transitions and events (`OnStateChanged`), owned by the engine (not a global), queryable by systems and behaviours; `Editing` is removed (a future tool mode would be a separate host, not an engine state).
- [x] **Simulation pausing and time:** one `SimulationClock` with time scale, pause, single-step (one fixed step while paused) and per-domain participation — physics, animation, particles, behaviours and audio each follow or ignore pause (UI and the camera keep running). Fixed-step accumulation with a clamp; deterministic step counts in tests.
- [x] **Tags:** `ObjectTag` becomes a proper tag set (several tags per entity, hashed names with a registry, fast queries `ForEachWithTag`, add/remove through the scene command buffer), used by gameplay and the sandbox (e.g. "Spawned", "Light", "Physics").
- [x] **Behaviours:** the behaviour registry and lifecycle (`OnCreate/OnStart/OnUpdate/OnFixedUpdate/OnPause/OnResume/OnDestroy`), per-behaviour state masks (runs while playing, paused or always), deterministic order, safe entity mutation through the command buffer; the fly/editor camera survives as a runtime **camera controller behaviour** (the same movement and controls as today).
- [x] **Commands:** the command registry stays for debug/console use.

### Exit criteria

- [x] no OpenGL or editor code, dependency or option remains; the verifier enforces it;
- [x] ~~the transitional sandbox still runs on the transitional Vulkan renderer …~~ superseded: Phases 22 and 23 landed together, so the sandbox runs on the modern renderer with the camera controller, pause/step and tags working;
- [x] engine state, clock, tags and behaviours have unit tests (state transitions, pause participation, single-step, tag queries, behaviour order and state masks).

---

## Phase 23 — Engine assembly and the sandbox demo

**Runtime and demo delivered 2026-09-25; desktop validation and the items below still open** (see [Engine runtime](EngineRuntime.md) and its findings). Put the modern systems together into the actual runtime and prove them in one real scene. This phase closes item 56 (normal rendering without CPU visible lists) and item 79's runtime wiring, and replaces the transitional renderer.

### Runtime assembly

- [x] The engine constructs the modern stack from runtime configuration: `RhiVulkan` device/swapchain (HDR when available), RenderGraph executor, residency (`AssetResidencyService`, `GeometryHeap`, `TextureResidency`, bindless tables), `GpuScene` + `RenderExtractor`, GPU visibility (frustum, HZB occlusion, LOD, binning, indirect draws), materials, clustered Forward+ (opaque and transparent), shadows, environment/IBL, post (exposure, bloom, grading), TAA, AO/SSR/fog, particles, skinning and animation, and the UI renderer with atlas residency.
  - [ ] still open: HDR swapchains, two-phase HZB occlusion in the frame, and the CPU animation runtime (item 78) driving skinned meshes.
- [x] One frame graph owned by a `FrameRenderer` with explicit views (main camera; shadow views; UI overlays; world UI among transparents, sorted and fogged).
  - [ ] still open: world UI is composited after post (depth-tested and distance-faded) rather than sorted among transparents and fogged.
- [x] `Engine::VulkanRenderer`, `LegacyRenderBinding`, `MeshPool`/`TexturePool`/`MaterialPool` and `FontPool`/legacy text are removed once the sandbox renders through the new path.
- [x] Scene/ECS components for the new systems: renderable (mesh, material instances), lights, shadows settings, environment, particles emitters, animators/skinned meshes, UI canvases (document, placement, surface, size, DPI, interactivity), cameras.
  - [ ] still open: environment and shadow settings live in `RenderSettings`, not components; there is no animator component yet (the sandbox poses skins from a behaviour).
- [x] Resize and minimize handled (swapchain resize requests; rendering paused while minimized); Linux builds and runs (SwiftShader).
- [ ] HDR toggle, device-loss recovery; the Windows build and run of the new runtime.
...
- [ ] **Controls panel** (screen canvas): play/pause/step/stop and time scale; physics backend and gravity; spawn menus (boxes, spheres, stacks, ragdoll-like chains, projectiles); lighting (sun direction/intensity, point/spot light spawner with color, radius, shadows on/off); shadows (cascade count, resolution, bias, cascade debug view); environment (IBL map dropdown, exposure); post (bloom, grading, TAA, AO, SSR, fog toggles and sliders); rendering debug views (cluster heatmap, visibility stats, wireframe, overdraw if available); material gallery selector. Uses dropdowns, radio groups, sliders with typed values, toggles, tooltips, list views (entity list with a virtual list), context menus on entities and a modal for reset/quit.
  - [x] delivered: play/pause/step/stop, time scale, camera bookmarks, ball drop/rain/fire, reset modal; every renderer switch, tone mapper, exposure, bloom, grading, sun, environment intensity and the cluster heat map; the entity browser (filter, virtual list, details, focus, delete) and spawn menu; the findings list. It uses buttons, toggles, checkboxes, sliders with typed values, a radio group, dropdowns, a text field, a list view, a virtual list, a scroll area, a menu, tooltips and a modal.
  - [ ] still open: physics backend and gravity, a light spawner, cascade/resolution/bias controls and cascade debug view, an IBL map dropdown, wireframe/overdraw views, entity context menus.
- [ ] **Physics playground:** throwable/stackable bodies, a ramp and a domino run, triggers, per-body inspection through the UI, PhysX and Jolt switchable.
  - [x] delivered: a box pyramid, a ramp with capsules, spheres and cubes, ball drop/rain/fire, spawnable shapes, impact counting from collision callbacks; `--physics` selects PhysX or Jolt at launch.
  - [ ] still open: a domino run, triggers, per-body inspection, switching backends at run time.
- [x] **Rendering playground:** PBR material spheres, a hall of instances for culling/LOD, animated skinned characters, particle emitters, transparent objects, emissive objects with bloom.
- [x] **World-space UI:** an in-world terminal panel and a billboard label, interactive through the ray-cast router.
- [x] **Camera:** the same movable fly camera as today (a behaviour), camera bookmarks, and pause not affecting it.
- [x] **Diagnostics overlay:** CPU/GPU frame times per pass, draw/instance counts, residency and memory, visibility statistics.
- [x] **Findings list:** a short document of what does not work or needs improvement, fed back into this plan.
...
56. [x] remove CPU-visible-list dependency from normal world rendering. *(2026-09-25, Phase 23: the runtime draws only from `GpuVisibility` indirect commands; `SceneBVH` and `VulkanIndexDraw` are archived. Two-phase HZB occlusion in the frame is still open.)*
...
83. [x] Remove Vulkan/OpenGL behavior from generic Vertex/Texture/Transform/Camera types. *(2026-09-25, Phase 22/23: the legacy `Vertex`/`Texture2D` types are archived, `Transform` lost its screen-space/backend modes and the runtime `Camera` has a single reverse-Z convention.)*
...
89. [x] Remove the OpenGL renderer, backend option and dependencies (Phase 22).
90. [x] Remove editor systems (gizmos, gizmo behaviours, editor-only commands, `Editing` state); the fly camera becomes a runtime camera-controller behaviour (Phase 22).
91. [x] Engine state machine, simulation clock (pause, time scale, single-step, per-domain participation), tag sets and the behaviour lifecycle with state masks (Phase 22).
92. [x] Assemble the modern runtime (`FrameRenderer`, residency, GPU Scene extraction, visibility, Forward+, shadows, post, TAA, screen-space effects, particles, skinning, UI) and retire the transitional renderer and legacy pools; closes items 56 and 79's runtime wiring (Phase 23). *(2026-09-25; desktop validation open — see Phase 23.)*
93. [x] Sandbox demo scene with the UI control panel, physics and rendering playgrounds, lighting/shadows, world UI, play/pause/step and the fly camera; findings list (Phase 23). *(2026-09-25; open extras listed in Phase 23 and the findings.)*
