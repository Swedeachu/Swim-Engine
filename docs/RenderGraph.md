...
Every live pass records a GPU label and, on queues supporting timestamps, a start/end timestamp pair. `ReadTimings()` waits before reading the query reset/write results and returns named durations in nanoseconds. Unsupported timestamp hardware returns absent durations. Timings include graph barriers inside the pass region; they are not CPU-calibrated clocks or evidence of async overlap. The begin timestamp is top-of-pipe, so a pass's duration can overlap the tail of earlier work. `EndOffsetNanoseconds` (end relative to the graph's first begin) is monotonic in schedule order, and the difference between consecutive passes' end offsets attributes GPU time to each pass without overlap.
