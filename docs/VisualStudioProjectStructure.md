...
| `SwimRhiSmokeShaders` | Build-only Slang artifact target under Tests; compiles the triangle/texture shaders and reflection sidecars when shader tooling and Vulkan RHI are enabled. |
| `SwimTests` | One executable containing pipeline/draw/descriptor/sampler capture tests and optional real-driver clear/triangle/texture smoke cases; depends on the shader artifact target when available. |

Shader artifacts are generated under the build directory. Rebuilding `SwimTests` updates its triangle and texture shaders through normal CMake dependencies. `SWIM_RUN_RHI_SMOKE=1` enables the desktop/GPU cases; ordinary tests do not require a Vulkan driver. The architecture plan's Phase 9 checkpoint lists the supported procedural/indexed and reflected sampled-2D paths, immutable descriptor-table lifetime rules, and remaining GPU/window validation gates.

`SwimShaderCompiler` also owns the tool-side `BuildRhiShaderInterface` conversion: parsed Slang metadata becomes an owned RHI descriptor schema. Its public dependency is the backend-neutral `SwimRhi` interface target; simdjson remains private. The texture smoke uses this conversion and the generated reflection sidecar. `SwimRhiVulkan` consumes the resulting generic schema and owns native layouts, descriptor pools/sets and samplers; it does not link the compiler or JSON parser. The descriptor implementation is split under `Backends/Vulkan/Descriptors`, with writes and command binding in separate implementation files.
