...
ClusteredLighting -> Lights, Materials (StandardPbr), RenderGraph, RHI contract (cluster grid, light assignment, heatmap)
Shadows        -> Visibility, GpuScene, Geometry, GpuMaterials, Materials, Lights, RenderGraph, RHI contract (cascades, shadow atlas, caster culling, PCF)
PostProcess    -> RenderGraph, RHI contract (exposure, bloom, grading, tone mapping)
Temporal       -> RenderGraph, RHI contract (jitter, TAA resolve, history)
ScreenSpace    -> RenderGraph, RHI contract (GTAO, depth-aware blur, reflections, height fog)
Particles      -> RenderGraph, RHI contract, Resources handles (GPU pool, simulation, sorting, indirect billboards)
Skinning       -> RenderGraph, RHI contract, Resources handles, Geometry (compute skinning + morphs into GeometryHeap output meshes)
Animation      -> Assets skeleton/clip structs, Jobs (Systems/Animation: sampling, blending, state machines, palettes)
ForwardPlus    -> Shadows, Visibility, GpuScene, Geometry, GpuMaterials, Materials, Environment, Lights, ClusteredLighting, RenderGraph, RHI contract (Clustered Forward+)
...
Scene, Render, UI, and Audio extraction remain future work; the modern Vulkan RHI and RenderGraph are separate foundation subtrees within `SwimEngine`, and the main game renderer is still transitional. RenderGraph, `Renderer/Resources`, `Renderer/Geometry`, `Renderer/GpuScene`, `Renderer/Visibility`, `Renderer/Materials`, `Renderer/GpuMaterials`, `Renderer/Environment`, `Renderer/Lights`, `Renderer/ClusteredLighting`, `Renderer/Shadows`, `Renderer/ForwardPlus`, `Renderer/PostProcess`, `Renderer/Temporal`, `Renderer/ScreenSpace`, `Renderer/Particles`, `Renderer/Skinning`, `Systems/Animation` and `Renderer/Residency` add no module project or runnable test target: their suites (`RenderGraph`, `RenderResources`, `RenderGeometry`, `RenderScene`, `RenderVisibility`, `RenderMaterials`, `RenderGpuMaterials`, `RenderEnvironment`, `RenderLights`, `RenderClusteredLighting`, `RenderShadows`, `RenderForwardPlus`, `RenderPostProcess`, `RenderTemporal`, `RenderScreenSpace`, `RenderParticles`, `RenderSkinning`, `Animation`, `RenderResidency`) join `SwimTests`, as does the EnTT render extraction suite in `Scene/Ecs`, and only `SwimRenderGraphPublicHeaders`, `SwimRenderResourcesPublicHeaders` and `SwimAnimationPublicHeaders` are separate compile boundaries under Tests/Header Boundary. See [the graph contract](RenderGraph.md), [GPU resource residency](GpuResidency.md), [GPU Scene](GpuScene.md), [GPU visibility](GpuVisibility.md), [Materials](Materials.md), [Environment](Environment.md), [GPU lights](Lights.md), [Clustered lighting](ClusteredLighting.md), [Shadows](Shadows.md), [Clustered Forward+](ForwardPlus.md), [Post-processing](PostProcess.md), [Temporal anti-aliasing](TemporalAntiAliasing.md), [Screen-space effects](ScreenSpace.md), [GPU particles](Particles.md) and [Animation and skinning](Animation.md).
