...
namespace Engine
{
	...
	private:
		...
		void ComputeFromMatrix(const glm::mat4& vp)
		{
			...
			// Canonical clip depth is [0,1]. Unlike the legacy [-1,1] extraction,
			// the near plane is row Z itself (z >= 0); the far plane remains W - Z.
			planes[4] = glm::vec4(vp[0][2], vp[1][2], vp[2][2], vp[3][2]); // Near
